...
namespace EmployeeTracker.Common.Data
{
	...
	public sealed class StringType : BaseType, ISerializable
	{
		...
		internal StringType(string oString) :
			base(DataType.String)
		{
			m_oString = oString;
		}

		internal StringType(IConvertible oIConvertible, IFormatProvider oIFormatProvider) :
			base(DataType.String)
		{
			m_oString = oIConvertible.ToString(oIFormatProvider);
		}

		internal StringType(SerializationInfo info, StreamingContext context) :
			base(DataType.String)
		{
				...
				else
				{
					switch (oIType.TypeCode)
					{
                        case TypeCode.DateTime :
							result = ToDateTime(oCultureInfo).CompareTo(oIType.ToDateTime(oCultureInfo));
                            break;
		...
		{
			return oDataType == DataType.String ? this : DataType.Create(oDataType, m_oString, oIFormatProvider);
		}

		public override IType ToType(DataType oDataType, string sFormat, IFormatProvider oIFormatProvider)
		{
			return oDataType == DataType.String ? this : DataType.Create(oDataType, m_oString, sFormat, oIFormatProvider);
		}
